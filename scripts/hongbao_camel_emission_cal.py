...
MONEY_SUPPLY         = 88888888888888888
GENESIS_BLOCK_REWARD     = 16000000000000000
FINAL_SUBSIDY = 400000000
COIN_EMISSION_MONTH_INTERVAL = 4 #months
...
def get_block_reward(height, coins_already_generated):
    if height < (PEAK_COIN_EMISSION_HEIGHT + COIN_EMISSION_HEIGHT_INTERVAL):
        ...
        money_supply_pct = 0.18 + interval_num*(0.023 + interval_num*0.0032)
...
def calculate_emssion_speed(print_by_year = False):
    ...
    while coins_already_generated < MONEY_SUPPLY - FINAL_SUBSIDY:
        ...
        if emission_speed_change_happened and (count % 2 if print_by_year else True):
            print format(height, '07'), "\t", '{0:.10f}'.format(block_reward/100000000.0), "\t", coins_already_generated/100000000.0, "\t", str(round(coins_already_generated*100.0/MONEY_SUPPLY, 2)), "\t\t", format(int(total_time/(60*60*24.0)), '04'), "\t", total_time/(60*60*24)/365.25
            f.write(format(height, '07') + "\t" + '{0:.8f}'.format(block_reward/100000000.0) + "\t" + str(coins_already_generated/100000000.0) + "\t" + '%05.2f'%(coins_already_generated*100.0/MONEY_SUPPLY) + "\t" + format(int(total_time/(60*60*24.0)), '04') + "\t" + str(round(total_time/(60*60*24)/365.25, 2)) + "\n")
        
        height += 1
        
    print format(height, '07'), "\t", '{0:.10f}'.format(block_reward/100000000.0), "\t", coins_already_generated/100000000.0, "\t", str(round(coins_already_generated*100.0/MONEY_SUPPLY, 2)), "\t\t", format(int(total_time/(60*60*24.0)), '04'), "\t", total_time/(60*60*24)/365.
    f.write(format(height, '07') + "\t" + '{0:.8f}'.format(block_reward/1000000000.0) + "\t" + str(coins_already_generated/100000000.0) + "\t" + '{0:.2f}'.format(round(coins_already_generated*100.0/MONEY_SUPPLY, 2)) + "\t" + format(int(total_time/(60*60*24.0)), '04') + "\t" + str(round(total_time/(60*60*24)/365.25, 2)) + "\n")
    
if __name__ == "__main__":
    f = open("hongbao_camel_emmission.txt", "w")
    calculate_emssion_speed()
    if COIN_EMISSION_MONTH_INTERVAL == 4:
        print "\n\n\n"
        f.write("\n\n\n")
