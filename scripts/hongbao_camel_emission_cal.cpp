...
#define FINAL_SUBSIDY                                   ((uint64_t)400000000) // 4 * pow(10, 8)
#define GENESIS_BLOCK_REWARD                            ((uint64_t)16000000000000000) // 18% dev mine

#define DIFFICULTY_TARGET                               215  // seconds

#define COIN_EMISSION_MONTH_INTERVAL                    4
  ...
  while (coins_already_generated < (MONEY_SUPPLY - FINAL_SUBSIDY)){
    ...
    if (height % COIN_EMISSION_HEIGHT_INTERVAL == 0){
      if (height < (PEAK_COIN_EMISSION_HEIGHT + COIN_EMISSION_HEIGHT_INTERVAL)){
        ...
        money_supply_percentage = 0.18 + interval_num*(0.023 + interval_num*0.0032);
    ...
    if (emission_speed_change_happened && (print_by_year ? count % 2 : true)){
      if (height == 0){
        std::cout << height << "\t"
          << std::fixed << std::setprecision(2) << block_reward / 100000000.0 << "\t"
          << std::setprecision(8) << coins_already_generated / 100000000.0 << "\t"
        ...
        std::cout << height << "\t"
          << std::fixed << std::setprecision(2) << block_reward / 100000000.0 << "\t"
          << std::setprecision(2) << coins_already_generated / 100000000.0 << "\t"
          << std::fixed << std::setprecision(2) << coins_already_generated*100.0 / MONEY_SUPPLY << "\t"
          << std::fixed << std::setprecision(0) << total_time / (60 * 60 * 24.0) << "\t"
          << std::setprecision(2) << total_time / (60 * 60 * 24.0) / 365.25 << "\n";
      }
